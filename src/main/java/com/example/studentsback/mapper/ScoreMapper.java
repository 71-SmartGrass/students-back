...
public interface ScoreMapper {
    ...
    // 6. 根据ID查询成绩（只查score表，不关联）
    @Select("SELECT * FROM score WHERE id = #{id}")
    Score selectById(Integer id);
    
    // 7. 根据学生ID查询成绩（关联学生和课程信息，填充Score实体类中的关联字段）
    @Select("SELECT " +
            "s.id, s.student_id, s.course_id, s.score, s.created_at as createTime, " +
            "stu.stu_name as studentName, " +
            "stu.stu_number as studentNumber, " +
